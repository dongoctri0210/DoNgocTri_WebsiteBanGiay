...
namespace WebsiteBanGiay.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Register(User _user)
        {
            ...
            {
                var check = ojbWebsiteBanGiayEntities.Users.FirstOrDefault(s => s.Email == _user.Email);
                if(check == null)
                {
                    _user.Password = GetMD5(_user.Password);
                    ojbWebsiteBanGiayEntities.Configuration.ValidateOnSaveEnabled = false;
                    ojbWebsiteBanGiayEntities.Users.Add(_user);
        ...
        public ActionResult Login(string email, string password)
        {
            if (ModelState.IsValid)
            {
                ...
                var data = ojbWebsiteBanGiayEntities.Users.Where(s => s.Email.Equals(email) && s.Password.Equals(f_password)).ToList();
                if (data.Count() > 0)
                {
                    //add session
